<template>
  <div v-if="!loading" class="space-y-4">
    ...
    <div ref="manualActionsSection">
      <ManualWebhookActions />
    </div>
  </div>
...
import ManualWebhookActions from '~/components/repo/ManualWebhookActions.vue';
...
const manualActionsSection = ref<HTMLElement | null>(null);
onMounted(async () => {
  ...
  loading.value = false;
  focusSectionIfRequested(route.query.section);
});
...
function focusSectionIfRequested(section: unknown) {
  if (section === 'webhooks') {
    nextTick(() => {
      manualActionsSection.value?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });
  }
}
...
watch(
  () => route.query.section,
  (section) => {
    focusSectionIfRequested(section);
  },
);
